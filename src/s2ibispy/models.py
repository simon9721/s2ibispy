"""Copied models module into package (full upstream content).

This file is a near-identical copy of the original top-level
`models.py` but updated to import `s2ibispy.s2i_constants` so it
works inside the `src/s2ibispy` package layout.
"""
...
class IbisModel:
    ...
    # Data / results
    ramp: IbisRamp = field(default_factory=IbisRamp)
...
class IbisPin:
    ...
    series_model: Optional[IbisModel] = None
...
class IbisTOP:
    ...
    mList: List[IbisModel] = field(default_factory=list)
...
"""Copied models module into package."""
