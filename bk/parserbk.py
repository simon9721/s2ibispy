...
class S2IParser:
    def __init__(self):
        ...
        self.tempDiffPin = IbisDiffPin(invPin="", vdiff=IbisTypMinMax(), tdelay=IbisTypMinMax())
    ...
    def process_key(self, key: str, args: str, line_num: int):
        ...
        if key == "vinl" and self.modelProc:
            self.tempModel.Vinl = self.match_num(args, line_num)
            return

        if key == "vinh" and self.modelProc:
            self.tempModel.Vinh = self.match_num(args, line_num)
            return

        if key == "vmeas" and self.modelProc:
            self.tempModel.Vmeas = self.match_num(args, line_num)
            return

        if key == "cref" and self.modelProc:
            self.tempModel.Cref = self.match_num(args, line_num)
            return

        if key == "rref" and self.modelProc:
            self.tempModel.Rref = self.match_num(args, line_num)
            return

        if key == "vref" and self.modelProc:
            self.tempModel.Vref = self.match_num(args, line_num)
            return
    ...
    def process_data(self, line: str, current_section: str, line_num: int):
        ...
        # pin_name inv_pin vdiff tdelay_typ  [tdelay_min tdelay_max]
            ...
            self.tempDiffPin = IbisDiffPin(
                invPin=inv_pin,
                vdiff=vdiff,
                tdelay=IbisTypMinMax(typ=tdelay_typ, min=tdelay_min, max=tdelay_max)
            )
